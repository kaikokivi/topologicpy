...
class Vertex():
    ...
    def IsInternal(vertex, topology, tolerance: float = 0.0001, silent: bool = False) -> bool:
        ...
        if Topology.IsInstance(topology, "Vertex"):
            ...
            proj_v = Vertex.Project(vertex, topology, mantissa=16)
            if Vertex.Distance(vertex, proj_v, mantissa=16) > tolerance:
                return False
            normal = Face.Normal(topology, mantissa=16)
            ...
            if any(abs(Vector.Dot(Vector.ByVertices(vertex, faceVertex, normalize=False, mantissa=16), normal, mantissa = 16)) > tolerance for faceVertex in Face.Vertices(topology)):
                return False
            
            periphery = Topology.Edges(topology)
            peripheral = any([(
                    Topology.Boolean(vertex, p, 'intersect') or 
                    Vertex.Distance(vertex, p, mantissa=16) <= tolerance)
                for p in periphery])
